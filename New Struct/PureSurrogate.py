...
class SurrogateStateBlockData(StateBlockData):
    ...
    def initialize(self):
        ...
        self.vapor_phase = Block() # Link pp
        self.liquid_phase = Block() # Link pp


        # Liquid surrogate block
        if self.params.surrogate_liq is not None:
            self.liquid_phase.surrogate = SurrogateBlock()
            self.liquid_phase.surrogate.pysmo = PysmoSurrogate.load(StringIO(self.params.surrogate_liq))
            self.liquid_phase.surrogate.build_model(
                self.liquid_phase.surrogate.pysmo,
                input_vars=input_list,
                output_vars=output_list,
            )

        # Vapor surrogate block
        if self.params.surrogate_vap is not None:
            self.vapor_phase.surrogate = SurrogateBlock()
            self.vapor_phase.surrogate.pysmo = PysmoSurrogate.load(StringIO(self.params.surrogate_vap))
            self.vapor_phase.surrogate.build_model(
                self.vapor_phase.surrogate.pysmo,
                input_vars=input_list,
                output_vars=output_list,
            )

            # Enable equilibrium calculation
            # Solve using ideal gas EOS?
            # Determine phase / in VLE?
