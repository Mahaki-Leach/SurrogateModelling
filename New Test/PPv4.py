...
class _SurrogateStateBlock(StateBlock):
    ...
    def initialize(blk, state_args=None, state_vars_fixed=False,
        ...
        for i in blk:
            # if blk[i].q.value == 0:
            #     print("Liquid surrogate")
            #     blk[i].surrogate.build_model(
            #         blk.params.config.liquid_surr,
            #         input_vars=[blk[i].T_s, blk[i].P_s],
            #         output_vars=[blk[i].enth_s, blk[i].entr_s, blk[i].z_s]
            #     )
            # elif blk[i].q.value == 1:
            #     print("Vapor surrogate")
            #     blk[i].surrogate.build_model(
            #         blk.params.config.vapor_surr,
            #         input_vars=[blk[i].T_s, blk[i].P_s],
            #         output_vars=[blk[i].enth_s, blk[i].entr_s, blk[i].z_s]
            #     )
            # else:
            print("Two-phase surrogate")
            blk[i].surrogate.build_model(
                blk.params.config.twophase_surr,
                input_vars=[blk[i].T_s, blk[i].P_s, blk[i].q],
                output_vars=[blk[i].enth_s, blk[i].entr_s]
            )
...
class SurrogateStateBlockData(StateBlockData):
    ...
    def build(self):
        ...
        # --- Input scaling expressions ---
        self.T_scale_link = Constraint(
            expr=self.T_s == (self.temperature - self.params.config.T_min) /
                            (self.params.config.T_max - self.params.config.T_min)
        )
        self.P_scale_link = Constraint(
            expr=self.P_s == (self.pressure - self.params.config.P_min) /
                            (self.params.config.P_max - self.params.config.P_min)
        )

        # self.enth_mol_expr = Expression(
        #     expr=self.params.config.enth_min + self.enth_s *
        #         (self.params.config.enth_max - self.params.config.enth_min)
        # )

        # self.entr_mol_expr = Expression(
        #     expr=self.params.config.entr_min + self.entr_s *
        #         (self.params.config.entr_max - self.params.config.entr_min)
        # )
...
class SurrogateParameterData(PhysicalParameterBlock):
    ...
    CONFIG.declare("T_min", ConfigValue(default=279, domain=float))
    CONFIG.declare("T_max", ConfigValue(default=534, domain=float))
    CONFIG.declare("P_min", ConfigValue(default=4849, domain=float))
    CONFIG.declare("P_max", ConfigValue(default=3417374, domain=float))
    CONFIG.declare("enth_min", ConfigValue(default=-1e5, domain=float))
    CONFIG.declare("enth_max", ConfigValue(default=1e5, domain=float))
    CONFIG.declare("entr_min", ConfigValue(default=-1e3, domain=float))
    CONFIG.declare("entr_max", ConfigValue(default=1e3, domain=float))
    CONFIG.declare("z_min", ConfigValue(default=0.0, domain=float))
    CONFIG.declare("z_max", ConfigValue(default=1.0, domain=float))
