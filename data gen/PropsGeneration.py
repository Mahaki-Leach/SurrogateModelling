...
# Loop in chunks
for span in range(size//spanLength):

    val_range = slice(span * spanLength, (span + 1) * spanLength)

    T = randDB[val_range.start]
    P = randP[val_range.start]
    ...
    try:
        ...
        for row in range(val_range.start, val_range.stop):
